...
//TCP connection: Sending data from serverA to client
	...
	for(p = servinfo; p != NULL; p = p->ai_next)
              {
		if ((client3 = socket(p->ai_family, p->ai_socktype,
				p->ai_protocol)) == -1)
               {
...
//-------------connect to the client----------------------------------------------------------------
         ...
         if (connect(client3, p->ai_addr, p->ai_addrlen) == -1) 
                {
	...
	printf("The Server C is up and running\n");
		...
		getsockname(client3,(struct sockaddr *)&local,&(locallen));
        port=ntohs(local.sin_port);
        
       
       // printf("\nThe Server C has IP address %s and port number %d\n",inet_ntoa(local.sin_addr),port);
...
printf("\nThe Server C has the following neighbor information:\n");
    ...
    send(client3,send_cost,strlen(send_cost),0);
	printf("\nThe Server C finishes sending its neighbor information to the Client with TCP port number %s and IP address %s\n",PORT,s);
	printf("\nFor this connection with the Client, the Server C has TCP port number %d and IP address %s.",port,s);
	close(client3);
...
inet_ntop(p->ai_family,addr,ipstr,sizeof (ipstr));
 ...
 memset(local.sin_zero, 0,sizeof (local.sin_zero));
		getsockname(client3_UDP,(struct sockaddr *)&local,&(locallen));
        port=ntohs(local.sin_port);
//-----------------------creation of socket--------------------------------------------------------------------------------
    for(p = servinfo; p != NULL; p = p->ai_next) 
     {
        if ((client3_UDP = socket(p->ai_family, p->ai_socktype,
                p->ai_protocol)) == -1) 
        {
        ...
        if (bind(client3_UDP, p->ai_addr, p->ai_addrlen) == -1)
       {
            close(client3_UDP);
 ...
 // ("listener: waiting to recvfrom...");
        ...
        addr_len = sizeof UDP;
    if ((numbytes = recvfrom(client3_UDP, buf, MAXBUFLEN-1 , 0,
        (struct sockaddr *)&UDP, &addr_len)) == -1) {
        perror("recvfrom");//receiving port number from patient
        exit(1);
    }
	buf[numbytes]='\0';
	printf("\nThe server C has received the network topology from the Client with UDP port number %s and IP address %s as follows:\nEdge------Cost\n",PORT,s);
	printf("%s",buf);
