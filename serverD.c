...
//TCP connection: Sending data from serverD to client
	...
	for(p = servinfo; p != NULL; p = p->ai_next)
              {
		if ((client4 = socket(p->ai_family, p->ai_socktype,
				p->ai_protocol)) == -1)
               {
...
//-------------connect to the client----------------------------------------------------------------
         ...
         if (connect(client4, p->ai_addr, p->ai_addrlen) == -1) 
                {
	...
	printf("The Server D is up and running\n");
		...
		getsockname(client4,(struct sockaddr *)&local,&(locallen));
...
printf("\nThe Server D has the following neighbor information:\n");
	  ...
	  printf("%s",send_msg);
        ...
        send(client4,send_cost,strlen(send_cost),0);
		
		
        
        sprintf(port_sent,"%d",port);
	printf("\nThe Server D finishes sending its neighbor information to the Client with TCP port number %s and IP address %s\n",PORT,s);
	printf("\nFor this connection with the Client, the Server D has TCP port number %d and IP address %s.",port,s);
	close(client4);
	memset(&hints, 0, sizeof hints);
...
inet_ntop(p->ai_family,addr,ipstr,sizeof (ipstr));
 ...
 memset(local.sin_zero, 0,sizeof (local.sin_zero));
		getsockname(client4_UDP,(struct sockaddr *)&local,&(locallen));
        port=ntohs(local.sin_port);
//-----------------------creation of socket--------------------------------------------------------------------------------
    for(p = servinfo; p != NULL; p = p->ai_next) 
     {
        if ((client4_UDP = socket(p->ai_family, p->ai_socktype,
                p->ai_protocol)) == -1) 
        {
        ...
        if (bind(client4_UDP, p->ai_addr, p->ai_addrlen) == -1)
       {
            close(client4_UDP);
 ...
 // ("listener: waiting to recvfrom...");
        ...
        addr_len = sizeof UDP;
    if ((numbytes = recvfrom(client4_UDP, buf, MAXBUFLEN-1 , 0,
        (struct sockaddr *)&UDP, &addr_len)) == -1) {
        perror("recvfrom");//receiving port number from patient
        exit(1);
    }
	buf[numbytes]='\0';
	printf("\nThe server D has received the network topology from the Client with UDP port number %s and IP address %s as follows:\nEdge------Cost\n",PORT,s);
	printf("%s",buf);
